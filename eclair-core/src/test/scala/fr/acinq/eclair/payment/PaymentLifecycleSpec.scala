...
class PaymentLifecycleSpec extends BaseRouterSpec {
  ...
  test("payment failed (unparsable failure)") { case (router, _) =>
    ...
    routerForwarder.expectMsg(RouteRequest(defaultAmountMsat, a, d, ignoreNodes = Set.empty, ignoreChannels = Set.empty))
    ...
    routerForwarder.expectMsg(RouteRequest(defaultAmountMsat, a, d, ignoreNodes = Set(c), ignoreChannels = Set.empty))

    // let's simulate a response by the router with another route
    sender.send(paymentFSM, RouteResponse(hops, 55L, Set(c), Set.empty))
    awaitCond(paymentFSM.stateName == WAITING_FOR_PAYMENT_COMPLETE)
  ...
  test("payment failed (local error)") { case (router, _) =>
    ...
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    val WaitingForRoute(_, _, Nil) = paymentFSM.stateData
    routerForwarder.expectMsg(RouteRequest(defaultAmountMsat, a, d, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set.empty))
    routerForwarder.forward(router)
    awaitCond(paymentFSM.stateName == WAITING_FOR_PAYMENT_COMPLETE)
    val WaitingForComplete(_, _, cmd1, Nil, _, _, _, hops) = paymentFSM.stateData
    ...
    routerForwarder.expectMsg(RouteRequest(defaultAmountMsat, a, d, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set(ChannelDesc(channelId_ab, a, b))))
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
  }
  ...
  test("payment failed (PermanentChannelFailure)") { case (router, _) =>
    ...
    // payment lifecycle forwards the embedded channelUpdate to the router
    awaitCond(paymentFSM.stateName == WAITING_FOR_ROUTE)
    routerForwarder.expectMsg(RouteRequest(defaultAmountMsat, a, d, assistedRoutes = Nil, ignoreNodes = Set.empty, ignoreChannels = Set(ChannelDesc(channelId_bc, b, c))))
    routerForwarder.forward(router)
  ...
  test("filter errors properly") { case _ =>
    val failures = LocalFailure(RouteNotFound) :: RemoteFailure(Hop(a, b, 0L, 0L, channelUpdate_ab) :: Nil, ErrorPacket(a, TemporaryNodeFailure)) :: LocalFailure(AddHtlcFailed("00" * 32, "00" * 32, ChannelUnavailable("00" * 32), Local(None), None)) :: LocalFailure(RouteNotFound) :: Nil
    val filtered = PaymentLifecycle.transformForUser(failures)
    assert(filtered == LocalFailure(RouteNotFound) :: RemoteFailure(Hop(a, b, 0L, 0L, channelUpdate_ab) :: Nil, ErrorPacket(a, TemporaryNodeFailure)) :: LocalFailure(ChannelUnavailable("00" * 32)) :: Nil)
  }
}
